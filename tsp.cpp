                   ...
                   unsigned nthread = 1)
{
  ...
  std::vector<std::thread> threads;
  for (unsigned i = 0; i < nthread; ++i) {
    threads.emplace_back(run_one_thread);
  }
...
//  }
  ...
  const auto cities = Cities("challenge.tsv");//Cities(argv[1]);
  const auto pop_size = 1000;//atoi(argv[2]);
  const auto mut_rate = 0.2;//atof(argv[3]);
  const auto nthread = 2;//(argc > 4)? atoi(argv[4]) : 1;
  const auto granularity = 100;//(argc > 5)? atoi(argv[5]) : 100;
...
//  const auto best_ordering = threaded_randomized_search(cities, NUM_ITER, 1);
